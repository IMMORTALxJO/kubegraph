...
class KubeGraph:
	...
	def collect_services(self):
		if self.options["namespace"]:
			svcs_list = self.clients['v1'].list_namespaced_service(self.options["namespace"], label_selector=self.options["svc_label_selector"],watch=False).items
		else:
	...
	def collect_ingresses(self):
		if self.options["namespace"]:
			ingresses_list = self.clients['v1beta1'].list_namespaced_ingress(self.options["namespace"],label_selector=self.options["ingress_label_selector"], watch=False).items
		else:
		...
		for ingress in ingresses_list:
			proto = 'http'
			if 'alb.ingress.kubernetes.io/certificate-arn' in ingress.metadata.annotations:
				proto = 'https'
			elif 'alb.ingress.kubernetes.io/listen-ports' in ingress.metadata.annotations:
				annotations = json.loads(ingress.metadata.annotations[
						'alb.ingress.kubernetes.io/listen-ports'])
				for annotation in annotations:
					if 'HTTPS' in annotation:
						proto = 'https'
			for rule in ingress.spec.rules:
				services = set()
				for path in rule.http.paths:
					services.add('%s.%s' % (path.backend.service_name, ingress.metadata.namespace))
				self.ingresses['%s://%s' % (proto, rule.host)] = services
	...
	def is_string_address(self, namespace, value, cache={}):
		...
		if value in self.services:  # mysqlserver
			...
			try:
				...
				pass
		if parsed_hostname and not parsed_hostname in ('0.0.0.0', '127.0.0.1'):
	...
	def generate_graph(self):
		...
		for (service_long_name,svc) in self.services.items():
